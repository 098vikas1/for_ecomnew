(function($) {
    ...
    var halo = {
            ...
            if (title && price && link && image) {
                ...
                if (enableMobileDots) {
                    ...
                    document.querySelector("[data-lookbook-popup-mobile-wrapper]").addEventListener("transitionend", () => {
                        selectedLookbookProduct.scrollIntoView({
                            block: "nearest",
                            behavior: "smooth"
                        });
                    });
        ...
        initVideoPopup: function() {
            if ($(".video-open-popup ").length) {} else {
                return
            }
            ...
            if (window.wishlist.show) {
                ...
                $doc.on('click', '[data-wishlist]', (event) => {
                    ...
                    if (!$target.hasClass('wishlist-added')) {
                        ...
                        localStorage.setItem('wishlistItem', JSON.stringify(wishlistList));
                        console.log($(['data-wishlist-items-grid-display']).hasClass('productList'));
        ...
        wishlistPagination: function() {
            ...
            wlpaggingContainer.attr('data-current-page', 1);
            let paggingTpl
            ...
            let wishlistItemDisplay = $('[data-wishlist-items-grid-display]');
            if (!wishlistItemDisplay.hasClass('productList')) return;
            ...
            wlpaggingContainer.off('click.wl-pagging').on('click.wl-pagging', 'li a', function(e) {
                ...
                wlpaggingContainer.attr('data-current-page', pageNumber);
        ...
        initWishlistPage: async function() {
            if (typeof(Storage) !== 'undefined') {
                ...
                if (wishlistList.length > 0) {
                    ...
                    wishlistList.forEach((handle, index) => {
                        halo.setProductForWishlistPage(handle, index, wishlistList.length);
                    })
                } else {
                    $('[data-wishlist-container]')
                        .addClass('is-empty')
        ...
        setProductForWishlistPage: function(handle, index, wishlistListLength) {
            var wishlistContainer = $('[data-wishlist-container]');

            $.ajax({
                type: 'get',
                url: window.routes.root + '/products/' + handle + '?view=ajax_wishlist_card',
                beforeSend: function() {},
                success: function(data) {

                    wishlistContainer.find('[data-wishlist-items-grid-display]').append(data);
                    const item = wishlistContainer.find(`[data-wishlist][data-wishlist-handle=${handle}]`);
                    item.addClass('wishlist-added').find('.text').text(window.wishlist.added);
                    item.addClass('is-in-grid');
                    $(data).attr('data-product-handle', handle);
                ...
                complete: function() {
                    ...
                    if (index >= 3) {
                        const item = wishlistContainer.find(`[data-wishlist][data-wishlist-handle=${handle}]`);
                        item.closest('.product').hide();
                    }

                    if (index === wishlistListLength - 1) {
                        wishlistContainer.find('[data-wishlist-items-grid-display]').addClass('is-loaded');
                        halo.wishlistPagination();
        ...
        initLazyloadObserver(loadingImages, productGrid) {
            ...
            if (productGrid) parentObserver.observe(productGrid, {
                subtree: false,
                childList: true
            });
        ...
        calculateTranslateYHeight() {
            const cardActions = document.querySelectorAll('.card .card-action')
            let heighest = 0;
